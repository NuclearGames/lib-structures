    ...
    public class PriorityQueue<TElement, TPriority> {
        ...
        public PriorityQueue(int initialCapacity, IComparer<TPriority>? comparer) {
            if (initialCapacity < 0) {
                throw new ArgumentOutOfRangeException($"Parameter {initialCapacity} must be non-negative");
            }
        ...
        private void Grow(int minCapacity) {
            ...
            if ((uint)newcapacity > ARRAY_MAX_LENGTH)
                newcapacity = ARRAY_MAX_LENGTH;
                        ...
                        if (en.MoveNext()) {
                            ...
                            while (en.MoveNext()) {
                                if (count == arr.Length) {
                                    ...
                                    if ((uint)newLength > ARRAY_MAX_LENGTH) {
                                        newLength = ARRAY_MAX_LENGTH <= count ? count + 1 : ARRAY_MAX_LENGTH;
                                    }
